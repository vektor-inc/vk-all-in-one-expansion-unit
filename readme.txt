...
[ Bug Fix ][ SNS Share Button ] Fixed the Threads and Copy icons not being displayed on themes that do not load Font Awesome (e.g. Twenty Twenty-Five), since the icons depended on Font Awesome even though the plugin itself does not load it. They are now drawn with inline SVG instead.
