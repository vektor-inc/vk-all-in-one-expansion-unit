...
[ Spec Change ][ Page Top Button ] Renamed the `--ver_page_top_button_url` CSS custom property to `--veu_page_top_button_url` to align with the `--veu_` (vk Ex Unit) naming convention. The legacy `--ver_` name is still emitted and used as the primary value, with the new `--veu_` name as a fallback, so themes and custom CSS that override `--ver_page_top_button_url` continue to work without any change.
