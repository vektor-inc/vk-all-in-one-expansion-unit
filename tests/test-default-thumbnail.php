...
class DefaultThumbnailTest extends WP_UnitTestCase {
	...
	/**
	 * Test that default thumbnail is not applied in media library context.
	 */
	function test_media_library_default_thumbnail_exclusion() {

		print PHP_EOL;
		print '------------------------------------' . PHP_EOL;
		print 'test_media_library_default_thumbnail_exclusion' . PHP_EOL;
		print '------------------------------------' . PHP_EOL;
		...
		// メディアライブラリのリスト表示をシミュレート
		// WP_Media_List_Table::column_title() の処理を再現
		$attachments = array( $this->default_thumbnail_attachment_id, $this->post_thumbnail_attachment_id );
