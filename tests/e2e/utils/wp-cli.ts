...
/**
 * 投稿を強制削除する。「対象が見つからない」旨の失敗は無視する。
 *
 * 【背景】以前は `cta.spec.ts` の `resetPosts()` が `post_type=post,cta` を種別ごと
 * 一括削除しており、ローカルの並列実行で他 spec ( 本ファイルを使う
 * cta-image-position-xss.spec.ts 等 ) の作業中の投稿を巻き込んで削除することがあった。
 * その根本原因は `cta.spec.ts` 側で「自分が作った投稿のタイトルだけを対象にする」
 * よう修正済みだが、その他の予期しない要因（環境固有の事情等）で対象が既に
 * 存在しないケースが今後も起こり得るため、後片付けが「対象は既に無かった」という
 * 良性の理由で失敗した場合に、実際のアサーション結果を覆い隠して FAIL 扱いに
 * しないための保険として本ヘルパーは残す。
 *
 * 【握りつぶす条件についての注意】`Failed deleting post {id}.` は wp-cli の
 * `Post_Command` が `wp_delete_post()` の戻り値が falsy だった時に出す汎用の
 * 失敗メッセージで、「対象が既に無い」場合と「`pre_delete_post` フィルタ等による
 * 削除拒否」の場合とで文言が区別できない。どちらであっても後片付けの失敗で
 * テスト結果を覆い隠さないために握りつぶすが、後者だとしたら見えないまま
 * 残骸が溜まる可能性があるため、握りつぶした内容は必ずログへ出力する
 * ( 現状維持でよいと判断された理由と対処は #1439 のセキュリティレビュー参照 )。
 * それ以外のエラー（コンテナ不在・接続不可等）はそのまま呼び出し元へ伝播させる。
 *
...
export const deletePostsTolerateMissing = ( ids: number[] ): void => {
	...
	try {
		...
		// 「対象が既に無い」以外の理由 ( 削除拒否等 ) でも同じ文言になり区別できないため、
		// 残骸に気づけるよう握りつぶした内容を必ず出力する。
		console.warn(
			`deletePostsTolerateMissing: 後片付けの削除に失敗しました ( 無視 ): ${ message }`
		);
	}
};
