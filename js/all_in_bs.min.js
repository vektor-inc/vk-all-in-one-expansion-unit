...
(function($){var facebook={init:function(){var url='//graph.facebook.com/?id='+encodeURIComponent(location.href);$.ajax({url:url,dataType:'jsonp',success:function(json){var count=json.shares?json.shares:0;$('.veu_socialSet').find('.veu_count_sns_fb').html(count);}});}}
...
(function($,d){var a=false,b='',c='',
    f=function(){
if(a){
    a=false;
    c.show();
    b.removeClass('active').text('<?php echo __( 'Edit Guide', 'vkExUnit' ).' : SHOW'; ?>');
}
else{a=true;c.hide();b.addClass('active').text('<?php echo __( 'Edit Guide', 'vkExUnit' ).' :  HIDE'; ?>');}
};
$(d).ready(function(){
    b=$('#wp-admin-bar-veu_disable_admin_edit .ab-item').on('click',f);c=$('.veu_adminEdit');
});
