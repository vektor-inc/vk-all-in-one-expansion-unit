(function(wp){
	...
	registerBlockType( 'vk-blocks/veu-call-page', {
		title: __('Call Page','vk-all-in-one-expansion-unit' ),
		icon: (
			<svg
				height="25"
				viewBox="0 0 24 25"
		...
		edit: ( props ) => {
			...
			let editContent;
			if ( TargetPost === -1 ) {
				editContent = <p>{ __( 'Because no post is selected, The block Will not render', 'vk-all-in-one-expansion-unit' ) }</p>;
			} else {
			...
			return (
				<Fragment>
					<InspectorControls>
						<PanelBody
							title={ __( 'Page Option', 'vk-all-in-one-expansion-unit' ) }
							initialOpen={ true }
						>
							<BaseControl
								id={ 'veu-call-01' }
							>
								<SelectControl
									label={ __( 'Select Page', 'vk-all-in-one-expansion-unit' ) }
									value={ TargetPost }
									options={ veu_call_pages }
